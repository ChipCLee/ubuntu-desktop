...
from franka_sim.franka_protocol import (
    ...
    SetEEToKCommand,  # Added import
    SetJointImpedanceCommand,
    SetLoadCommand,
    SetNEToEECommand,  # Added import
...
class FrankaSimServer:
    ...
    def __init__(self, host="0.0.0.0", port=COMMAND_PORT, enable_vis=False, genesis_sim=None):
        ...
        # kGetRobotModel (12) uses the .dat file (URDF representation)
        self.robot_model_dat_path = Path(__file__).parent / "robot.urdf"
        # kLoadModelLibrary (11) uses the .so file (binary library)
        # Mr. Chen, you will need to provide this 'model_library.so' file.
        self.model_library_so_path = Path(__file__).parent / "model_library.so"
    ...
    def reset_state(self):
        ...
        # --- ADDED FIX ---
        # Explicitly reset the simulator's control mode to a known default (POSITION).
        # This prevents the simulator's mode from the previous session
        # from "leaking" into the new connection.
        if self.genesis_sim:
            logger.info("Resetting simulator control mode to POSITION.")
    ...
    def handle_move_command(self, client_socket, header: MessageHeader, payload: bytes) -> None:
        ...
        try:
            ...
            # --- FIX: REMOVED SIMULATOR MODE SETTING ---
            # The UDP loop (_handle_commands) is now the single source of truth
            # for setting the simulator's control mode based on the robot_state.
            # This prevents the race condition you identified.
    ...
    def handle_set_load_command(self, client_socket, header: MessageHeader, payload: bytes):
        ...
        try:
            ...
            logger.debug(f"Mass: {cmd.m_load}")
            logger.debug(f"Center of mass: {cmd.F_x_Cload}")
            logger.debug(f"Inertia: {cmd.I_load}")

            # TODO: Update robot state with these values
    ...
    def handle_set_ne_to_ee_command(self, client_socket, header: MessageHeader, payload: bytes):
        ...
        try:
            ...
            logger.debug(f"NE_T_EE Matrix: {cmd.NE_T_EE}")

            # Update robot state
            self.robot_state.state["NE_T_EE"] = cmd.NE_T_EE
            logger.info("Robot state NE_T_EE transform updated.")
    ...
    def handle_set_ee_to_k_command(self, client_socket, header: MessageHeader, payload: bytes):
        ...
        try:
            ...
            logger.debug(f"EE_T_K Matrix: {cmd.EE_T_K}")

            # Update robot state
            self.robot_state.state["EE_T_K"] = cmd.EE_T_K
            logger.info("Robot state EE_T_K transform updated.")
    ...
    def handle_load_model_library_command(
        ...
        """
        Handle LoadModelLibrary command by sending the model file.
        
        Protocol requires:
        1. Header (12 bytes): <III> (command, command_id, total_size)
        2. Payload:
           a. File Data (N bytes): Raw file bytes
        
        (Based on user feedback, this command sends *only* the file data
         as the payload, with no preceding status or length fields).
        """
        try:
            # The payload is typically the model type (e.g., uint16_t for 0)
            model_type = struct.unpack("<H", payload)[0]
            logger.info(f"Received LoadModelLibrary command for model type: {model_type}")

            file_data = b''
            response_command = Command.kLoadModelLibrary
            response_command_id = header.command_id
            header_size = 12
            
            if not self.model_library_so_path.exists():
                logger.critical(
                    "CRITICAL: Model library file NOT FOUND at path: %s",
                    self.model_library_so_path.resolve()
                )
                # If file not found, send failure.
                # Send a 1-byte failure status, similar to GetRobotModel.
                status_bytes = struct.pack("<B", 1) # kCommandNotPossibleRejected
                payload_resp = status_bytes
                total_size = header_size + len(payload_resp)
                
            else:
                # File exists, read it
                with open(self.model_library_so_path, "rb") as f:
                    file_data = f.read()
                
                # Send kSuccess status (0)
                status_bytes = struct.pack("<B", 0) # <--- 1 byte status
                
                # Per user request, payload is status + file data.
                payload_resp = status_bytes + file_data
                
                # Total size is header + payload size
                total_size = header_size + len(payload_resp)
                logger.info(
                    f"Successfully loaded robot model file ({len(file_data)} bytes). "
                    f"Total payload size: {len(payload_resp)} bytes."
                )

            # Create the standard 12-byte header
            response_header_bytes = struct.pack(
                "<III", 
                response_command.value, 
                response_command_id, 
                total_size
            )
            
            # Combine header and payload into a single message
            full_response = response_header_bytes + payload_resp
            
            # Send the complete message in one call
            client_socket.sendall(full_response)
            
            logger.info(
                f"Sent LoadModelLibrary response (Total Size: {total_size} bytes)."
            )

        except Exception as e:
            logger.error(f"Error handling LoadModelLibrary command: {e}", exc_info=True)
            # Try to send a failure response on error
            try:
                # Send a header + 1-byte failure status
                status_bytes = struct.pack("<B", 1) # kCommandNotPossibleRejected
                total_size = header_size + 1 # 1 byte for status
                error_header = struct.pack(
                    "<III", 
                    response_command.value, 
                    response_command_id, 
                    total_size
                )
                client_socket.sendall(error_header + status_bytes)
            except Exception as e2:
                logger.error(f"Failed to send error response: {e2}")

    def handle_get_robot_model_command(
        self, client_socket, header: MessageHeader
    ):
        """
        Handle GetRobotModel command by sending the model file.
        
        Protocol requires:
        1. Header (12 bytes): <III> (command, command_id, total_size)
        2. Payload:
           a. Status (1 byte): <B> (1-byte status)
           b. File Data (N bytes): Raw file bytes
        """
        logger.info("Processing GetRobotModel command...")
        
        file_data = b''
        response_command = Command.kGetRobotModel
        response_command_id = header.command_id
        header_size = 12
        
        # Payload structure: 1-byte status + N-byte data
        status_payload_size = 1

        try:
            if not self.robot_model_dat_path.exists():
                logger.critical(
                    "CRITICAL: Robot model file NOT FOUND at path: %s",
                    self.robot_model_dat_path.resolve()
                )
                # If file not found, send failure status
                # kCommandNotPossibleRejected is 1
                status_bytes = struct.pack("<B", 1) # <--- 1 byte status
                file_data = b''
                
                payload = status_bytes + file_data # <--- No file_len_bytes
                total_size = header_size + len(payload)
                
            else:
                # File exists, read it
                with open(self.robot_model_dat_path, "rb") as f:
                    file_data = f.read()
                
                # Send kSuccess status (0)
                status_bytes = struct.pack("<B", 0) # <--- 1 byte status
                
                # DO NOT send file length in the payload for this command
                ...
                logger.info(
                    f"Successfully loaded robot model file ({len(file_data)} bytes). "
                    f"Total payload size: {len(payload)} bytes."
                )
            ...
            # Combine header and payload into a single message
            full_response = response_header_bytes + payload
            
            # Send the complete message in one call
            client_socket.sendall(full_response)
            
            logger.info(
                f"Sent GetRobotModel response (Total Size: {total_size} bytes)."
            )

        except Exception as e:
            logger.error(f"Error handling GetRobotModel command: {e}", exc_info=True)
            # Try to send a failure response on error
            try:
                # Send a header + failure status
                status_bytes = struct.pack("<B", 1) # kCommandNotPossibleRejected
                total_size = header_size + status_payload_size
                error_header = struct.pack(
                    "<III", 
                    response_command.value, 
                    response_command_id, 
        ...
        while self.running:  # Keep the TCP thread running even after client disconnects
            try:
                ...
                if header.command == Command.kMove:
                    ...
                    self.handle_get_robot_model_command(client_socket, header)
                ...
                elif header.command == Command.kSetNEToEE:  # Added handler
                    logger.info("Handling SetNEToEE command")
                    self.handle_set_ne_to_ee_command(client_socket, header, payload)
                elif header.command == Command.kSetEEToK:  # Added handler
    ...
    def run_server(self):
        ...
        try:
            ...
            # --- START WINDOWS FIX ---
            # Add check for SO_REUSEPORT, as it's not available on Windows
