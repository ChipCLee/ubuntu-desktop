...
class SetLoadCommand:
    ...
    m_load: float  # mass of the load
    F_x_Cload: list[float]  # 3 elements for center of mass
    I_load: list[float]  # 9 elements for inertia matrix

    @classmethod
    def from_bytes(cls, data: bytes) -> "SetLoadCommand":
        """Parse SetLoad command from binary data"""
        # Expected size: 1*8 (m_load) + 3*8 (F_x_Cload) + 9*8 (I_load) = 104 bytes
        if len(data) != 104:
            raise ValueError(
                f"Invalid payload size for SetLoadCommand. Expected 104, got {len(data)}"
            )

        offset = 0
        m_load = struct.unpack("<d", data[offset : offset + 8])[0]
        offset += 8
        F_x_Cload = list(struct.unpack("<3d", data[offset : offset + 24]))
        offset += 24
        I_load = list(struct.unpack("<9d", data[offset : offset + 72]))
...
class SetNEToEECommand:
    ...
    NE_T_EE: list[float]  # 16 elements for 4x4 column-major matrix

    @classmethod
    def from_bytes(cls, data: bytes) -> "SetNEToEECommand":
        """Parse SetNEToEE command from binary data"""
        # Expected size: 16 * 8 = 128 bytes
        if len(data) != 128:
            raise ValueError(
                f"Invalid payload size for SetNEToEECommand. Expected 128, got {len(data)}"
            )
        
        NE_T_EE = list(struct.unpack("<16d", data[:128]))
        return cls(NE_T_EE)
...
class SetEEToKCommand:
    ...
    EE_T_K: list[float]  # 16 elements for 4x4 column-major matrix

    @classmethod
    def from_bytes(cls, data: bytes) -> "SetEEToKCommand":
        """Parse SetEEToK command from binary data"""
        # Expected size: 16 * 8 = 128 bytes
        if len(data) != 128:
            raise ValueError(
                f"Invalid payload size for SetEEToKCommand. Expected 128, got {len(data)}"
            )
        
        EE_T_K = list(struct.unpack("<16d", data[:128]))
        return cls(EE_T_K)